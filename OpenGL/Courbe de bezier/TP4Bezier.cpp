...
CourbeBezier * test = new CourbeBezier();
...
unsigned int pas = 100;
 long  double pas2 = 0.01;
  vector<point3>   tableau;
...
GLuint leVBObezier;//pour afficher les points de contrôle
...
static void RenderScene()
{
  ...
  glColor3f(0.4, 0.4, 0.4);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, leVBObezier);
    glVertexPointer(3, GL_FLOAT, 0, 0); //description des données pointées

  glDrawArrays(GL_LINE_STRIP, 0, tableau.size()); //les éléments à utiliser pour le dessin
...
static void CreateVertexBuffer2()
{
    ...
    tableau = test->Bernstein(pas, controlPointList);//on stock dans un vecteur de point tout les points de bernstein de la class test
  unsigned m = 0;//va utilisé comme compteur pour naviguer dans le tableau
  float  vertices[3*tableau.size()];//est le tableau qui va recolter les points de bezier

	 for (std::vector<point3>::iterator it = tableau.begin(); it != tableau.end(); ++it)
   {
      vertices[m] = (*it).x;//on rentre la valeur de x des points dans le tableau
      std::cout << vertices[m] << std::endl ;//j'affiche la valeur de x pour montrer que les valeur de ma courbes sont bon
      vertices[m+1] = (*it).y;//on rentre la valeur de x des points dans le tableau
      std::cout << vertices[m + 1] << std::endl ;//j'affiche la valeur de x pour montrer que les valeur de ma courbes sont bon
      vertices[m+2] = (*it).z;
      std::cout << vertices[m + 2] << std::endl ;//j'affiche la valeur de x pour montrer que les valeur de ma courbes sont bon
      m += 3;
    }
  glGenBuffers(1, &leVBObezier); //génération d'une référence de buffer object
  glBindBuffer(GL_ARRAY_BUFFER, leVBObezier); //liaison du buffer avec un type tableau de données
  glBufferData(GL_ARRAY_BUFFER, sizeof(float)*tableau.size()* 3, vertices, GL_STATIC_DRAW); //création et initialisation du container de données (size() sommets -> 3*size() floats)

}

static void CreateVertexBuffer()
{
	float vertices2[3*controlPointList->size()]; //sommets à 3 coordonnées x,y,z par point
	unsigned m = 0;
  ...
  glGenBuffers(1, &leVBO); //génération d'une référence de buffer object
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float)*controlPointList -> size() * 3, vertices2, GL_STATIC_DRAW); //création et initialisation du container de données (size() sommets -> 3*size() floats)
