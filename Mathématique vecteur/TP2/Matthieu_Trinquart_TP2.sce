...
function a=TRIANGLE(droite , triangle )//la fonction prend en paramètre 2 points d'une droite et les 3 points du triangles
    
    I = intersectionplan(droite,triangle)//retourne la position du point d'intersection entre la droite et le plan du triangle 
         if(I == "Le plan n existe pas")
...
//---calcule la somme des petits parallèlogrammes former par IP0 et IP , IP1 et IP2 et IP2 et IP0----//
                 petitparrallelogramme1 = produitvectoriel(IP0,IP1) ;
                 petitparrallelogramme1 = sqrt((petitparrallelogramme1(1))^2 + petitparrallelogramme1(2)^2 +petitparrallelogramme1(3)^2)
                 petitparrallelogramme2 = produitvectoriel(IP1,IP2) ;
                 petitparrallelogramme2 = sqrt((petitparrallelogramme2(1))^2 + petitparrallelogramme2(2)^2 +petitparrallelogramme2(3)^2)
                 petitparrallelogramme3 = produitvectoriel(IP2,IP0) ;
                 petitparrallelogramme3 = sqrt(petitparrallelogramme3(1)^2 + petitparrallelogramme3(2)^2 +petitparrallelogramme3(3)^2)
                 sommepetittriangle = petitparrallelogramme1 + petitparrallelogramme2 + petitparrallelogramme3 ;
                 produitvectoriel(IP0,IP1) + produitvectoriel(IP1,IP2)+ produitvectoriel(IP2,IP0)
            
//---calcule le parallèllogramme former par les 3 points du triangles----//
                 grandparallelograme = produitvectoriel(P0P1,P0P2)
                 grandparallelograme = sqrt((grandparallelograme(1))^2 + grandparallelograme(2)^2 +grandparallelograme(3)^2)
//---si la somme des petits parallélogramme est égales au grand parrallélogramme provoquer par les 3 points du triangle alors la droite touche le triangle---// 
                 if(sommepetittriangle == grandparallelograme)
...
//--creation des 2 points d'une droite qui touche le triangle-------------//
pointD1touche = [1/3,1/3,2]
pointD2touche= [1/3,1/3,1]

touche = [ pointD1touche ,pointD2touche ]
//-------------------------------------------------------------------------//


//--creation des 2 points d'une droite qui rate le triangle-------------//
pointD1rater = [0,2,2]
pointD2rater = [0,2,1]

rater = [ pointD1rater ,pointD2rater ]
//-------------------------------------------------------------------------//


//--creation des 2 points d'une droite qui est parrallèlle au triangle-------------//
pointD1par = [0,0,1]
pointD2par = [5,0,1]

parra = [ pointD1par ,pointD2par ]
...
//--si vous voulez que la droite touche le triangle mettez "touche" a la premiere valeur de la fonction TRIANGLE
//--si vous voulez que la droite rate le triangle mettez "rater" a la premiere valeur de la fonction TRIANGLE
//--si vous voulez que la droite rate le triangle mettez "rater" a la premiere valeur de la fonction TRIANGLE
//--si vous voulez que la droite soit parrallèle au triangle mettez "parra" ou "parra2" a la premiere valeur de la fonction TRIANGLE
//  /!\ c'est point de droite marche que avec les point de planP
test = TRIANGLE(parra2 , planP );
disp(test);
