...
*/
	if (pidRetour > 0)
	{
		...
		if(pidRetour3 > 0)
		{
			cout << "Processus père (pidRetour=" << pidRetour3 << "), mon pid est " << getpid() << endl;
			sleep(20); //attente de 20 secondes. Attention, à ne pas utiliser en dehors des tests
		}else if(pidRetour3 == 0)
		{
			cout << "Processus fils 3 (pidRetour=" << pidRetour3 << "), mon pid est  " << getpid() << ", le pid de mon père est " << getppid() << endl;
			sleep(20); //idem
		}
	...
	else if (pidRetour == 0)
	{
		...
		if(pidRetour2 > 0)
		{
			cout << "Processus fils 1 (pidRetour=" << pidRetour2 << "), mon pid est  " << getpid() << ", le pid de mon père est " << getppid() << endl;
			sleep(20); //idem
		}else if(pidRetour2 == 0)
		{
			cout << "Processus fils 2 (pidRetour=" << pidRetour2 << "), mon pid est  " << getpid() << ", le pid de mon père est " << getppid() << endl;
			sleep(20); //idem
		}
